...
* The book handles the stack of compilers with an explicit linked list, and lets globals implicitly take care of shared state. Initially I used the Rust call stack to handle the compiler stack, and explicitly copied around shared state. This broke down with the introduction of closures. The easiest solution I found is ... not very elegant, to say the least:
  * The shared, "global" state is stored in a single instance of a state object behind an `Rc<RefCell<_>>`. This is OK; it makes some code a bit verbose, but fine.
  * To encode "resolve variables on the next compiler up the stack", I gave up on actually storing a reference to the enclosing compiler and calling `resolve variable` on it. Instead, I manually maintain a stack of `locals`es on the shared state. This is horrible, but it's enough to support closures, and doesn't need a *complete* refactor of how `Compiler`s are handled.
